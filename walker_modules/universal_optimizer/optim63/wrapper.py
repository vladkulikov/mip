...
class CurrentVariants:
    cfg_file_name = 'optimizator_config.json'
    ...
    def create_config(self, config):
        ...
        cfg_config = {
            ...
            self.method: {
                ...
                'accuracy_x': config['accuracy_x'],
                'accuracy_y': config['accuracy_y'],
                #                'n_sampling': config['n_sampling'],
                'sampling_method': config['sampling_method'],
                'number_of_ineffective_steps': config['count_ineff'],
                'objectives': [{'name': y_value.name, 'obj_type': 'min', 'weight': 1} for y_value in self.y_values if
                               y_value.type == 0]
            }
        }

        cfg_config = update_config(config, cfg_config, self.x_values, self.y_values)

        with open(self.cfg_file_name, 'w') as file:
            file.write(json.dumps({'optimization': cfg_config}, indent=4))
